...
// Запись часовой средней температуры
void log_hourly_temperature(double average_temperature) {
    std::lock_guard<std::mutex> lock(db_mutex); // Защищаем доступ к базе данных
    ...
    const char* insert_query = "INSERT INTO hourly_logs (timestamp, average_temperature) VALUES (strftime('%s', 'now'), ?);";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, insert_query, -1, &stmt, nullptr) != SQLITE_OK) {
        std::cerr << "Ошибка подготовки запроса: " << sqlite3_errmsg(db) << std::endl;
        sqlite3_close(db);
    ...
    sqlite3_bind_double(stmt, 1, average_temperature);
...
// Запись дневной средней температуры
void log_daily_temperature(double average_temperature) {
    std::lock_guard<std::mutex> lock(db_mutex); // Защищаем доступ к базе данных
    ...
    const char* insert_query = "INSERT INTO daily_logs (timestamp, average_temperature) VALUES (strftime('%s', 'now'), ?);";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, insert_query, -1, &stmt, nullptr) != SQLITE_OK) {
        std::cerr << "Ошибка подготовки запроса: " << sqlite3_errmsg(db) << std::endl;
        sqlite3_close(db);
...
void start_http_server() {
    ...
    svr.Get("/temperature/hourly_statistics", [](const httplib::Request& req, httplib::Response& res) {
        auto start = std::time(nullptr) - 60; // 1 минута назад
        auto end = std::time(nullptr);
        auto logs = get_temperature_logs(start, end);
        ...
        double average_temperature = logs.empty() ? 0.0 : sum / logs.size();

        std::ostringstream oss;
        oss << "{\"timestamp\":" << std::time(nullptr) << ",\"average_temperature\":" << average_temperature << "}";
        res.set_content(oss.str(), "application/json");
    });

    svr.Get("/temperature/daily_statistics", [](const httplib::Request& req, httplib::Response& res) {
        auto start = std::time(nullptr) - 24 * 60; // 24 минуты назад
        auto end = std::time(nullptr);
        auto logs = get_temperature_logs(start, end);
